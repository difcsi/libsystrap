...
/* Per-thread in-emulation marker (defined in sigill.c). The clone-child and
 * sigreturn resume paths below leave handle_sigill via a direct rt_sigreturn
 * (they never reach its out: label), so they clear this marker themselves. */
...
Z||Z'|________|/    incl. saved ip = clone site (1)    |________|/    incl saved ip = clone site (AGAIN; YES)
 ...
 v___|__-_-_-_|/ <-- sp at time of syscall             |__-_-_-_|/ <-- hacked sp we give to clone()
	...
	if (ret_op == 0 && __systrap_clone_child_direct_sigreturn)
	{
		 ...
		 * unwinding handle_sigill's epilogue. See the matching comment in
		 * do_clone3. Use the child's own current SP (on the new thread stack),
		 * 16-byte aligned; RSP for the syscall must be &uc (kernel reads the
		 * frame at RSP-8). */
		uintptr_t child_sp;
...
#error "Unsupported architecture."
	...
	if (ret_op == 0 && __systrap_clone_child_direct_sigreturn)
	{
		 ...
		 * here races (observed: the child resuming on the *parent's* stack ->
		 * corruption). Use the child's own current SP, which is on the new thread
