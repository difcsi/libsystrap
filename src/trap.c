...
/* ------------------------------------------------------------------
 * Trap-site registry and SIGILL handler chaining.
 *
 * We replace syscall instructions with ud2 to trap them. But other code in
 * the process may legitimately use ud2 for its own purposes -- notably
 * Alaska's GC-style safepoints, which patch ud2 into poll points and install
 * their own SIGILL handler. To coexist, we record every site WE replaced; on
 * SIGILL, only faults at a recorded site are treated as our syscall traps. Any
 * other ud2 (or non-ud2 SIGILL) is forwarded to the client handler registered
 * via __systrap_set_sigill_handler().
 *
 * The registry is an open-addressing hash set in a fixed-size mmap'd region,
 * so lookups (on the hot syscall-trap path) are O(1) and the reads are
 * async-signal-safe: no locking, no allocation. Entries are never removed.
 * ------------------------------------------------------------------ */
...
void __systrap_record_trap_site(uintptr_t addr)
{
	...
	trap_sites[i] = addr;
	++trap_sites_count;
	/* If we ever approach capacity the O(1) assumption breaks; that would be a
	 * bug (far more syscall sites than expected). */
	...
	set_trap_fn_t *set_trap, void *set_trap_arg)
{
	...
	if (shdrs)
	{
		...
		else debug_printf(5, "in executable mapping %p-%p, could not get last section boundary\n",
			begin, end);
		...
		else debug_printf(5, "in executable mapping %p-%p, could not get first section boundary\n",
			begin, end);
	}
